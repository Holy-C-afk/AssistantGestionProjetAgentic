...
function TagPopover({ pos, tags, onAddTag, onClose }) {
...
export default function TaskCard({ task, onClick, onPriorityChanged, isAdmin = true }) {
  ...
  /* Open priority menu — capture fixed position from the button */
  ...
  const openTagPop = (e) => {
    ...
    const rect = e.currentTarget.getBoundingClientRect();
    // Try to open above; if not enough space, open below
  ...
  const handleAddTag = async (newTag) => {
    ...
    try {
      ...
      onPriorityChanged?.(); // refresh board
    } catch (e) {
      console.error(e);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 mt-2">
          ...
          <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-3)' }}>
            ...
            {/* Tag icon — always visible for admins; shows count badge if tags exist */}
            {isAdmin && (
              <button
                onPointerDown={e => e.stopPropagation()}
                onClick={openTagPop}
                disabled={savingTag}
                title={localTags.length > 0 ? `${localTags.length} tag(s) · Ajouter` : 'Ajouter un tag'}
      ...
      {/* Priority dropdown — rendered in body via portal to avoid overflow clipping */}
      {showPrioMenu && (
        <PriorityDropdown
          pos={prioMenuPos}
      ...
      {/* Tag popover — rendered in body via portal */}
      {showTagPop && (
        <TagPopover
          pos={tagPopPos}
          tags={localTags}
