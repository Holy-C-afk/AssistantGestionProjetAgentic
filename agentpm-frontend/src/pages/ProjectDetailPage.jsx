  ...
  return (
    <div className="bg-gray-50 p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleExportPdf} disabled={pdfLoading}
              ...
              {pdfLoading
                ? <><span className="inline-block w-4 h-4 border-2 border-gray-300 border-t-indigo-600 rounded-full animate-spin" />Génération...</>
                : <>📄 Exporter PDF</>}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-6">
          {editing ? (
            <form onSubmit={handleUpdate}>
              <input className="w-full border rounded-lg px-3 py-2 mb-3 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
              <textarea className="w-full border rounded-lg px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                rows={3} placeholder="Description (optionnel)" />
              <div className="flex gap-3">
                <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">Sauvegarder</button>
                <button type="button" onClick={() => setEditing(false)} className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300">Annuler</button>
              </div>
            </form>
          ) : (
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => setEditing(true)} className="text-indigo-600 hover:underline text-sm ml-4">Modifier</button>
            </div>
          )}
        </div>

        <div className="flex gap-1 mb-4 border-b border-gray-200">
          {[{ k: 'board', l: 'Sprint Board' }, { k: 'members', l: 'Membres' }].map(t => (
        ...
        {tab === 'board' && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-1">
              <SprintSelector projectId={id} selectedSprintId={selectedSprintId} onSelect={setSelectedSprintId} />
            </div>
            <div className="lg:col-span-3">
              <KanbanBoard sprintId={selectedSprintId} projectId={id} refreshKey={boardRefreshKey}
        ...
        {tab === 'members' && (
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <form onSubmit={handleAddMember} className="flex gap-3 mb-6 flex-wrap">
              <input type="text" placeholder="User ID (UUID)" value={newUserId}
                onChange={e => setNewUserId(e.target.value)}
                className="flex-1 min-w-48 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
              ...
              <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700">Ajouter</button>
            </form>
            <div className="space-y-2">
              {members.length === 0 && <p className="text-gray-400 text-sm text-center py-6">Aucun membre pour l'instant.</p>}
              {members.map(m => (
                <div key={m.userId} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleRemoveMember(m.userId)} className="text-red-400 hover:text-red-600 text-sm">Retirer</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {selectedTaskId && (
          <TaskDetailModal taskId={selectedTaskId} onClose={() => setSelectedTaskId(null)} onUpdated={refreshBoard} />
        )}
